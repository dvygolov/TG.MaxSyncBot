...
- `TG_ADMIN_ID` — ID админа для команд боту в личке (`/start`, `/status`).
...
3. `TG_ADMIN_ID` (опционально, но рекомендуется для команд):
- числовой ID пользователя-админа, который может писать боту `/start` и `/status`.

4. `ADMIN_CHAT_ID` (опционально):
- можно поставить username канала/чата или числовой ID.
- для каналов числовой ID обычно вида `-100...`.

5. Дать боту права в исходном канале:
